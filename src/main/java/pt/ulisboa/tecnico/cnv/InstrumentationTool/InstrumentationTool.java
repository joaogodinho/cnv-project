...
public class InstrumentationTool {
	...
	public static final String usage = "Usage: java InstrumentationTool ." 
   ...
   /* recursive method that finds all class files under this directory */
		try {
			...
			for (int i = 0; i < ifnames.length; i++) {
				...
				if(tmpstr.equals(tmppath+"/"+InstrumentationTool.class.getName()+".class"))continue;
...
//								processFiles(file_tmp, tmppath);
					...
					/* java.util.Enumeration - std way to walk through a vector */
						...
						/* see BIT/lowBIT/Method_Info */
							...
							for(Enumeration bb = routine.getBasicBlocks().elements();bb.hasMoreElements();){
								BasicBlock b = (BasicBlock) bb.nextElement();
								b.addBefore("InstrumentationTool", "count", b.size());
							}
							for(Instruction instruction : routine.getInstructions()){
								if(InstructionTable.InstructionTypeTable[instruction.getOpcode()] == InstructionTable.CONDITIONAL_INSTRUCTION)
									instruction.addBefore("InstrumentationTool", "countConditions", 1);
							}
							if(routine.getMethodName().equals("main"))
								routine.addAfter("InstrumentationTool","printICount", "nothing");
							
						}
						ci.write(tmppath +"/output/"+ ifnames[i]);
					}
				}
			}
			return;
	 	...
	 	public static synchronized void printICount(String foo) {
	        System.out.println("prime was calculated in " + numberOfInstructions + " instructions ");
	        System.out.println("prime was calculated in " + numberOfConditionChecks + " conditional checks ");
	    }
